...
class Program
{
    // create a global refrence to a timer
    static Timer t;
    // create a global bool timerTrigger
    ...
    start:
        ...
        for (nCntr = 0; nCntr < nQuestions; ++nCntr)
        {
            ...
            do
            {
                ...
                // create a new timer with a 5 second wait time
                t = new Timer(5000);
                // delegate an event handeler to the time is up method
                ...
                {
                    // dont output correct input type if they are incorect becoues of time
                    if (!timeUp)
                    {
                        Console.WriteLine("Please enter an integer.");
                        bValid = false;
                ...
                // breakout of the loop if they ran out of time
                if (timeUp)
                {
                    break;
                }
            ...
            // if they ran out of time intentionally enter the wrong answer
            if (timeUp)
            {
                nResponse = nAnswer + 1;
            }
            // reset the timerTrigger
            timeUp = false;
    ...
    /* Method: TimeIsUp
     * Purpose: To trigger the timer and inform the user that their time has expired
     * Restrictions: None
     */
    static void TimeIsUp(Object source, System.Timers.ElapsedEventArgs e)
    {
        // output a message for the user to know that time is up
        Console.WriteLine("Your time is up, better luck next time.");
        // set the timeTrigger
        timeUp = true;
        // stop the timer
        t.Stop();
